...
class TransformerInferencer:
    ...
    def initialize(self):
        ...
        try:
            ...
            if 'num_encoder_layers' in config and 'num_decoder_layers' in config:
                # New format from seq2seq single run
                total_layers = config['num_encoder_layers'] + config['num_decoder_layers']
            else:
                # Original format - infer from total layers
                total_layers = config.get('num_layers', 6)
