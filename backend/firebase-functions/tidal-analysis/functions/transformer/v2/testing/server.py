    ...
    def handle_inference_request(self):
        ...
        try:
            ...
            if action == 'status':
                ...
                # Predict from a training sequence
                ...
                if 'index' not in params:
                    ...
                    self.send_header('Content-type', 'application/json')
                    self.end_headers()
                    self.wfile.write(json.dumps({'error': 'Missing sequence index parameter'}).encode())
                    return
                ...
                # Load training data (since that's what's displayed in the interface)
