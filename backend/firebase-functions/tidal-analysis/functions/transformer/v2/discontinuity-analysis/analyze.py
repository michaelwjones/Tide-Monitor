#!/usr/bin/env python3
"""
Transformer v2 Discontinuity Analysis Tool
...
class DiscontinuityAnalyzer:
    ...
    def analyze_inference_data(self, analysis_types: List[str], num_tests: int = 50):
        """Analyze inference discontinuities"""
        print("Analyzing Inference Discontinuities")
        print("=" * 40)
        ...
        # Initialize data packager with same normalization params
        try:
            self.data_packager = DataPackager(str(norm_params_path))
            print("Data packager initialized")
        except Exception as e:
            print(f"Error initializing data packager: {e}")
            return
            
        # Load Firebase data using shared data packager (handles freshness automatically)
        try:
            firebase_data = self.data_packager.load_fresh_firebase_data()
            print(f"Loaded {len(firebase_data)} Firebase readings")
        except Exception as e:
            print(f"Error loading Firebase data: {e}")
            return
        
        # Find test data that starts after validation period
        validation_end_time = self._find_validation_end_time()
        test_start_time = validation_end_time + timedelta(days=3)
        test_end_time = datetime.now().replace(tzinfo=validation_end_time.tzinfo)
        
        print(f"Validation ends: {validation_end_time.strftime('%Y-%m-%d')}")
        print(f"Testing period: {test_start_time.strftime('%Y-%m-%d')} to {test_end_time.strftime('%Y-%m-%d')}")
        print(f"Testing {num_tests} sequences at 1-minute intervals")
        
        discontinuities_found = []
        
        for i in range(num_tests):
            test_time = test_start_time + timedelta(minutes=i)
            
            try:
                # Create input sequence using shared data packager
                input_array, sequence_metadata = self.data_packager.create_inference_sequence(firebase_data, test_time)
                
                if len(input_array) != 432:
                    print(f"Error: Expected 432 points, got {len(input_array)}")
                    continue
                    
                # Run inference
                prediction = self.inference_engine.predict(input_array)
                
                # Detect discontinuities
                if 'level_jumps' in analysis_types:
                    last_actual = input_array[-1]  # Last value of input sequence
                    first_predicted = prediction[0]  # First value of prediction
                    level_result = self.detector.detect_level_jump(last_actual, first_predicted)
                ...
                if 'direction_changes' in analysis_types:
                    ...
                    actual_trend = self.detector.calculate_trend(input_array[-12:].tolist(), 12)
                    predicted_trend = self.detector.calculate_trend(prediction[:12].tolist(), 12)
                    direction_result = self.detector.detect_direction_reversal(actual_trend, predicted_trend)
                    if direction_result['detected']:
                        discontinuities_found.append({
                            'time': test_time.isoformat(),
                            'type': 'direction_reversal',
                            'details': direction_result
                        })
                        
                if 'cusps' in analysis_types:
                    # Use last 12 points of input and first 12 points of prediction
                    cusp_result = self.detector.detect_cusp(input_array[-12:].tolist(), prediction[:12].tolist())
                    if cusp_result['detected']:
                        discontinuities_found.append({
                            'time': test_time.isoformat(),
                            'type': 'cusp',
                            'details': cusp_result
            ...
            if (i + 1) % 10 == 0:
                print(f"Processed {i + 1}/{num_tests} tests, found {len(discontinuities_found)} discontinuities")
                
        print(f"\nAnalysis complete:")
        print(f"Tests run: {num_tests}")
        print(f"Discontinuities found: {len(discontinuities_found)}")
        print(f"Discontinuity rate: {len(discontinuities_found)/num_tests*100:.1f}%")
