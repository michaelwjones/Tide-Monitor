...
class TidalDataset(Dataset):
    ...
    def __init__(self, data_dir='../data-preparation/data', split='train', 
        ...
        # Load timestamps for reference (optional - for backward compatibility)
        timestamp_path = os.path.join(data_dir, 'timestamps.json')
        try:
            with open(timestamp_path, 'r') as f:
                self.timestamp_info = json.load(f)
        except FileNotFoundError:
            # Generate synthetic timestamp info if file doesn't exist
            self.timestamp_info = {
                'data_range': {
                    'start': 'Not available (synthetic timestamps)',
                    'end': 'Not available (synthetic timestamps)'
                }
            }
            print("  Timestamps: Using synthetic timestamps (timestamps.json not found)")
