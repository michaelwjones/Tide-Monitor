...
if not exist "..\..\local\training\checkpoints\best.pth" (
    echo ERROR: Trained PyTorch model not found!
    echo Please train the transformer model first using local/training/train_transformer.py
    echo Expected: local\training\checkpoints\best.pth
    echo.
    cd /d "%ORIGINAL_DIR%"
    pause
    exit /b 1
)
echo [OK] Trained PyTorch model found: local\training\checkpoints\best.pth

echo.
echo Copying model checkpoint to inference directory...
copy "..\..\local\training\checkpoints\best.pth" "best.pth" > nul
if %ERRORLEVEL% EQU 0 (
